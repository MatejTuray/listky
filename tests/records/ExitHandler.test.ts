...
describe('POST /exit', () => {
  ...
  beforeAll(async () => {
    const created = await supertest(app)
      .post('/api/tickets')
      .send({ owner: 'TestExit', type: 'ONE_TIME' });
  ...
  it('Should not create record when ticketId is not uuid v4', async () => {
    const response = await supertest(app).post('/api/exit').send({
  ...
  it('Should not create record when entry record doesnt exist', async () => {
    const response = await supertest(app).post('/api/exit').send({
      ticketId: testId,
      location: 'barfoo',
    });
  ...
  it('Should create exit record', async () => {
